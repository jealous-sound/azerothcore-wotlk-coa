...
private:
    static void Load()
    {
        ...
        auto path = sConfigMgr->GetOption<std::string>("Bloodforged.Catalogue", "");
        std::ifstream input(path);
        if (!input)
        {
            LOG_ERROR("server.loading", "Bloodforged disabled: catalogue cannot be read: {}", path);
            return;
        }
        ...
        while (std::getline(input, line))
        {
            if (line.empty() || line.front() == '#')
                continue;
            uint32 entry, quality, requiredLevel, itemLevel;
            std::string extra;
            std::istringstream row(line);
            if (!(row >> entry >> quality >> requiredLevel >> itemLevel) || row >> extra)
            {
                LOG_ERROR("server.loading", "Bloodforged disabled: malformed catalogue row");
                return;
            }
